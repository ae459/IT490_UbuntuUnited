...
function getTicketmasterEvents($req) {
    $params = array();

    $fields = array(
        "keyword", "city", "stateCode", "countryCode",
        "classificationName", "size", "page", "sort",
    ...
    $result = fetchTicketmasterEvents($params);

    return array(
        "type" => "ticketmaster_events_result",
        "request_id" => isset($req["request_id"]) ? $req["request_id"] : "no_id",
...
function requestProcessor($req) {
    ...
    if ($req["type"] == "get_ticketmaster_events") {
        return getTicketmasterEvents($req);
    }
